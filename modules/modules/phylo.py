...
# 共通モジュールからインポート
from modules.common import find_tool_path, generate_alignment_html_from_df
...
def app_phylo():
    ...
    # ツールパス探索
    mafft_def = find_tool_path("mafft")
    trimal_def = find_tool_path("trimal")
    iqtree_def = find_tool_path("iqtree") or find_tool_path("iqtree2")
    ...
    with st.expander("🔧 ツール詳細設定 (Parameters)", expanded=False):
        ...
        with tab1:
            ...
            c1, c2 = st.columns(2)
            mafft_binary = c1.text_input("MAFFT Path", value=mafft_def if mafft_def else "mafft")
        ...
        with tab2:
            st.caption("trimAl")
            trimal_binary = st.text_input("trimAl Path", value=trimal_def if trimal_def else "trimal")
        ...
        with tab3:
            st.caption("IQ-TREE")
            iqtree_binary = st.text_input("IQ-TREE Path", value=iqtree_def if iqtree_def else "iqtree2")
            c5, c6 = st.columns(2)
                ...
                with tempfile.TemporaryDirectory() as td:
                    ...
                    with st.spinner("Running MAFFT..."):
                        with open(out_aln, "w") as f:
                            subprocess.run(cmd_mafft, stdout=f, stderr=subprocess.PIPE, text=True)
                    ...
                    if use_trimal:
                        ...
                        if trimal_extra: cmd_trimal.extend(trimal_extra.split())
                            ...
                            if "-automated1" not in cmd_trimal: cmd_trimal.extend(["-gt", str(trimal_gt)])
                        
                        subprocess.run(cmd_trimal, capture_output=True)
        ...
        # Step 2: 確認・編集
            ...
            with c2:
                if st.button("🌳 IQ-TREEを実行", key="phylo_iqtree"):
                    sel = edf[edf["Include"]==True]
                    if len(sel) < 3: st.error("最低3配列必要です"); st.stop()
                    
                    with tempfile.TemporaryDirectory() as td:
                        aln = os.path.join(td, "aln.fa")
                        SeqIO.write([SeqRecord(Seq(r["Sequence"]), id=r["ID"], description="") for i,r in sel.iterrows()], aln, "fasta")
                        
                        cmd = [iqtree_binary, "-s", aln, "-bb", str(bootstrap), "-pre", os.path.join(td,"out"), "-nt", iqtree_nt]
