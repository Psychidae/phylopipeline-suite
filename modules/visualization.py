...
def create_main_figure(visible_results, alignment_ref, consensus_data, mismatches, target_pos, trace_win, 
    ...
    # 座標基準は常にアラインメントのリファレンスを使用
    ref_track = alignment_ref["display"]
    
    # --- 座標変換ヘルパー (Trace Index <-> Base Index) ---
    ...
    if num_visible == 0: total_rows += 1 # ダミー行

    # 行の高さ計算
    cons_height = 0.20 if show_translation else 0.10
    remain_h = 0.85 - (0.15 if has_orf else 0) - cons_height
    
    row_heights = []
    if has_orf: row_heights.append(0.15)
    row_heights.append(cons_height)
    
    if num_visible > 0:
        row_heights.extend([remain_h / num_visible] * num_visible)
    else:
        row_heights.append(remain_h) # ダミー
    ...
    # ==========================================
    # 2. Consensus & Translation (表示枠付き)
    # ==========================================
    # 背景枠の描画 (DNAエリア)
    fig.add_shape(type="rect", x0=x_inf_min, x1=x_inf_max, y0=0.6, y1=1.2, 
                  fillcolor="rgba(240, 240, 240, 0.5)", line_width=0, layer="below", row=consensus_row, col=1)
    
    # 背景枠の描画 (Translationエリア)
    if show_translation:
        fig.add_shape(type="rect", x0=x_inf_min, x1=x_inf_max, y0=-0.1, y1=0.6, 
                      fillcolor="rgba(255, 250, 240, 0.5)", line_width=0, layer="below", row=consensus_row, col=1)
        # 境界線
        fig.add_shape(type="line", x0=x_inf_min, x1=x_inf_max, y0=0.6, y1=0.6,
                      line=dict(color="gray", width=1, dash="dot"), row=consensus_row, col=1)
    ...
    # ==========================================
    # 3. Waveforms (Visible Only)
    # ==========================================
    colors = {'A': '#2ca02c', 'T': '#d62728', 'C': '#1f77b4', 'G': '#000000'}
    
    if num_visible == 0:
        fig.add_annotation(text="表示する波形データがありません", showarrow=False, row=waveform_start_row, col=1)
    else:
        # 表示する文字の密度判定
        ...
        for i, track in enumerate(visible_results):
            ...
            # --- アラインメント範囲推定 ---
            q_start_ref_idx = offset
            q_end_ref_idx = offset + len(d["sequence"])
            map_start = max(0, q_start_ref_idx)
            map_end = min(len(ref_peaks), q_end_ref_idx)
            ...
            fp = [ref_peaks[k] for k in ref_indices] # Ref X
            xp = query_peaks_mapped                  # Query X
            ...
            for b in ['A','T','G','C']:
                ...
                if len(y_raw) > 0: 
                    current_max = np.max(y_raw)
                    track_max_y = max(track_max_y, current_max)
                y_plot = y_raw[::step]
                
                fig.add_trace(go.Scatter(
                    x=x_plot, y=y_plot,
                    mode='lines', line=dict(color=colors[b], width=1),
            ...
            if show_waveform_text:
                ...
                for k in range(k_start, k_end):
                    ...
                    if 0 <= q_idx < len(d["sequence"]):
                        ...
                        track_x.append(px)
                        track_c.append(char)
                        track_col.append(t_col)

                if track_x:
                    fig.add_trace(go.Scatter(
                        x=track_x,
                        y=[text_y_pos] * len(track_x),
                        mode="text",
                        text=track_c,
            ...
            # Y軸範囲
            fig.update_yaxes(
                title_text=track["name"][:8], 
                range=[-track_max_y*0.1, track_max_y * 1.3], 
                showticklabels=False, fixedrange=True, row=row_idx, col=1
            )
    ...
    # レイアウト調整
    fig.update_layout(
        height=250 + 180 * max(1, num_visible), 
