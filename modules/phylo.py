...
def run_simple_asap(aligned_fasta_path, threshold=0.02):
    ...
    Kimura-2-Parameter (K2P) 距離などを計算し、閾値でクラスタリングする
    """
    # 配列読み込み
    ...
    # 行列計算のために数値化 (簡易的にp-distanceを使用)
    # 本格的なK2PはBiopythonのDistanceCalculator等を使うが、ここではscikit-learn用に自作
    ...
    # ギャップ(-)は無視するか、違いとしてカウントするか。ここでは単純なハミング距離/長さ
    def p_dist(s1, s2):
    ...
    # クラスタリング (Hierarchical)
    Z = linkage(squareform(dist_matrix), method='average') # UPGMA
    
    # 閾値でグループ分け (ASAP scoreの最小値探索は複雑なので、ユーザー指定閾値で切る)
